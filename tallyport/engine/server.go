...
func NewServer(
	serverName string, address string, logger zerolog.Logger, tlsDir string, mux http.Handler, opts ServerOpts) *Server {
...
func (s *Server) Serve() {
	...
	go func(s *Server, logger zerolog.Logger) {
		logger.Info().Msgf("Starting server (%s) on %v", s.ServerName, s.server.Addr)
		if !s.Opts.EnableTls {
